...
RenderSystem::RenderSystem(sf::RenderWindow& renderTarget)
...
void RenderSystem::run()
{
    ...
    if (debug)
    {
        ...
        for (size_t i = 0; i < m_Ents.size(); ++i)
        {
            ...
            getRenderTarget().draw(disp->sprite);
            ...
            if (m_Ents[i]->hasComp<Movement>())
            {
                ...
                info.setColor(sf::Color(0, 255, 40));
    ...
    else
    {
        for (size_t i = 0; i < m_Ents.size(); ++i)
        {
			getRenderTarget().draw(m_Ents[i]->comp<Display>()->sprite);
        }
    }
}

void RenderSystem::setRenderTarget(sf::RenderWindow& renderTarget)
{
	this->renderTarget = &renderTarget;
}

sf::RenderWindow& RenderSystem::getRenderTarget() const
{
	return *renderTarget;
}